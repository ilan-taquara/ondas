...
const Certificate = () => {
  ...
  const [hideButtons, setHideButtons] = React.useState(false); // Novo estado para controlar a visibilidade dos botões
  function excelSerialDateToJSDate(serial) {
    // Excel usa 1 como 01/01/1900, então subtraímos 1 do número de série.
    // No entanto, Excel considera 1900 como ano bissexto, o que não é.
    // Portanto, se o número de série é maior que 59, subtrai 1 adicionalmente.
  ...
  function formatDate(date) {
    ...
    let month = String(date.getMonth() + 1).padStart(2, '0'); // Meses são base 0
  ...
  const handleSubmit = async (event) => {
    ...
    const filtered = dataCertificate.filter((data) => {
      ...
      console.log(formattedDates); // Output: ["05/11/2023", "12/11/2023", "15/10/2023", "22/10/2023"]
  ...
  const handleGeneratePDF = (certificateId) => {
    ...
    setHideButtons(true); // Oculta os botões antes de capturar a imagem
    ...
    setTimeout(() => {
      html2canvas(certificateElement).then((canvas) => {
        ...
        setHideButtons(false); // Mostra os botões novamente após capturar a imagem
      });
    }, 100);
  };
        ...
        <div className="container">
          {data.map((certificate, index) => (
            ...
            >
              ...
              <p>{`Mês: ${month.value}`}</p>
              <p>{`Ano: ${year.value}`}</p>
              {!hideButtons && ( // Condicional para exibir/ocultar o botão
                <button
